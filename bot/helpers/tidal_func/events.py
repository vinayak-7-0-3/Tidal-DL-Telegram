    ...
    try:
        ...
        while elapsed < TIDAL_API.key.authCheckTimeout:
            ...
            await bot.edit_message_text(
                chat_id=c_id,
                message_id=msg.message.id, 
                text=lang.AUTH_SUCCESFULL_MSG.format(
...
=================================
'''
async def start(string, bot, msg, c_id, r_id):
    ...
    for item in strings:
        ...
        try:
            await start_type(etype, obj, bot, msg, c_id, r_id)
        except Exception as e:
            LOGGER.warning(str(e))

async def start_type(etype: Type, obj, bot, msg, c_id, r_id):
    if etype == Type.Album and Config.SEARCH_CHANNEL:
        check = await check_duplicate(obj.title, obj.artist, bot, c_id, r_id, etype)
        if check:
            return
    if etype == Type.Album:
        await start_album(obj, bot, msg, c_id, r_id)
    elif etype == Type.Track:
        await start_track(obj, bot, msg, c_id, r_id)
    elif etype == Type.Artist:
        await start_artist(obj, bot, msg, c_id, r_id)
    elif etype == Type.Playlist:
        await start_playlist(obj, bot, msg, c_id, r_id)
    elif etype == Type.Mix:
        await start_mix(obj, bot, msg, c_id, r_id)

async def start_mix(obj: Mix, bot, msg, c_id, r_id):
...
async def start_artist(obj: Artist, bot, msg, c_id, r_id):
    ...
    for item in albums:
        await start_album(item, bot, msg, c_id, r_id)
